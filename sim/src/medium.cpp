...
SIM_IMPORT_EXPORT
Medium::Medium() : controllers_count_(0)
{
  paused_.store(false);
  ...
  try {
    while (running_.load(std::memory_order_relaxed)) {
      ...
      do {
        ...
        real_duration = static_cast<uint64_t>(
          std::chrono::duration_cast<std::chrono::nanoseconds>(tp1 - tp0)
            .count());
