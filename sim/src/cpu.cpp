...
#define GR8(n)       r_file_[n]
#define IO_GR8(n)    io_space_[n]

#define GR16(n)      (*reinterpret_cast<uint16_t*>(r_file_ + n))
#define IO_GR16(n)   (*reinterpret_cast<uint16_t*>(io_space_ + n))
...
SIM_IMPORT_EXPORT AVRCore::AVRCore(
  MICROCONTROLLER mc,
  Flash&          flash,
  sram_t&         sram,
  eeprom_t&       eeprom,
...
inline int AVRCore::DecodeInstruction(
...
SIM_IMPORT_EXPORT void AVRCore::Decode(
  uint16_t begin_w, uint16_t end_w)
{
...
#ifdef _MSC_VER
SIM_IMPORT_EXPORT int32_t AVRCore::Draw(
  HDC hdc) const
{
  std::stringstream ss;
  ...
  ss << "SREG:"
        " I:"
     << (SREG & BIT(I) ? 1 : 0) << " T:" << (SREG & BIT(T) ? 1 : 0)
     << " H:" << (SREG & BIT(H) ? 1 : 0) << " S:" << (SREG & BIT(S) ? 1 : 0)
     << " V:" << (SREG & BIT(V) ? 1 : 0) << " N:" << (SREG & BIT(N) ? 1 : 0)
     << " Z:" << (SREG & BIT(Z) ? 1 : 0) << " C:" << (SREG & BIT(C) ? 1 : 0);
  draw_line();

  ss << "PC: 0x" << std::setw(4) << int(PC * 2) << " SP: 0x" << std::setw(4)
     << int(STACK);
  draw_line();

  for (int i = 0; i < 8; i++) {
    for (int j = 0; j < 4; j++) {
      ss << "R" << std::dec << std::setw(2) << int(8 * j + i) << " 0x"
         << std::hex << std::setw(2) << int(GR8(8 * j + i)) << "  ";
    }
    draw_line();
  }
  ...
  for (int i = 0x60, k = 1; i < 0x460; i++, k++) {
    if (k == 16) {
      k = 0;
      ss << std::setw(2) << int(GR8(i));
      draw_line();
    } else if (k % 4 == 0) {
      ss << std::setw(2) << int(GR8(i)) << " | ";
    } else {
      if (k == 1) {
        ss << std::setw(4) << i << "   " << std::setw(2) << int(GR8(i)) << " ";
      } else {
        ss << std::setw(2) << int(GR8(i)) << " ";
      }
    }
  ...
  for (size_t i = 0, k = 1; i < flash_size; i++, k++) {
    if (k == 8) {
      ...
      if (k == 1) {
        ss << std::setw(4) << i * 2 << "   " << std::setw(4)
           << int(flash_[i].instruction) << " ";
      } else {
        ss << std::setw(4) << int(flash_[i].instruction) << " ";
      }
    }
...
SIM_IMPORT_EXPORT void AVRCore::PrintRegisterFile() const
{
  ...
  for (int i = 0; i < 8; i++) {
    for (int j = 0; j < 4; j++) {
      printf("R%.2d 0x%.2x   ", 8 * j + i, GR8(8 * j + i));
    }
    std::cout << "\n";
  }
}
...
uint32_t AVRCore::do_spm()
{
  ...
  if (SPMCR & (1 << Atmega16::SPMEN)) {
    if (SPMCR & (1 << Atmega16::PGERS)) {
      ...
      Decode(static_cast<uint16_t>(page_begin_w),
             static_cast<uint16_t>(page_end_w));
...
SIM_IMPORT_EXPORT int AVRCore::Step()
{
  ...
  if ((SREG & (1 << I)) && !interrupts_.empty()) {
    uint16_t int_addr;
    bool     consumed =
      interrupts_.consume_one([&int_addr](uint16_t addr) { int_addr = addr; });
    if (consumed) {
  ...
  switch (op) {
    ...
    case OP_LDI:
      ...
      GR8(reg_n) = (instruction & 0x0F) | ((instruction >> 4) & 0xF0);
      PC         = PC + 1;
      CC         = CC + 1;
      break;
    case OP_COM:
    ...
    case OP_NEG:
      reg_n      = (instruction >> 4) & 0x1F;
      tmp8       = GR8(reg_n);
      GR8(reg_n) = ~GR8(reg_n) + 1;
      h          = ((1 << 3) & tmp8) | ((1 << 3) & GR8(reg_n));
      v          = GR8(reg_n) == 0x80 ? 1 : 0;
      n          = (1 << 7) & GR8(reg_n);
      z          = GR8(reg_n) ? 0 : 1;
      c          = !z;
      s          = n ^ v;
    ...
    case OP_ASR:
      ...
      GR8(reg_n) = (GR8(reg_n) >> 1) | (GR8(reg_n) | (1 << 7));
      n          = (GR8(reg_n) & 0x80) >> 7;
      z          = GR8(reg_n) ? 0 : 1;
      v          = n ^ c;
      s          = n ^ v;
...
#ifdef _MSC_VER
      _addcarry_u8(0, GR8(Rd), GR8(Rr), &GR8(Rd));
#else
    ...
    case OP_ROR:
      reg_n      = (instruction >> 4) & 0x1F;
      c          = GR8(reg_n) & 0x01;
      GR8(reg_n) = (GR8(reg_n) >> 1) | ((SREG & 0x01) << 7);
      n          = GR8(reg_n) & 0x80 ? 1 : 0;
      z          = GR8(reg_n) ? 0 : 1;
      v          = n ^ c;
      s          = n ^ v;
      SREG &= ~((1 << C) | (1 << N) | (1 << Z) | (1 << S) | (1 << V));
...
#ifdef _MSC_VER
      _addcarry_u8(GETF_CARRY(), GR8(Rd), GR8(Rr), &GR8(Rd));
#else
      __asm__ __volatile__(
        "bt %3, %2\n\t"
        "adc %1, %0\n\t"
        : "+r"(GR8(Rd))
        : "r"(GR8(Rr)), "r"((uint16_t)SREG), "i"(0)
        : "cc");
#endif
      eflags = __readeflags();
...
#ifdef _MSC_VER
      _subborrow_u8(0, GR8(Rd), GR8(Rr), &GR8(Rd));
#else
...
#ifdef _MSC_VER
      _subborrow_u8(GETF_CARRY(), GR8(Rd), GR8(Rr), &GR8(Rd));
#else
      __asm__ __volatile__(
        "bt %3, %2\n\t"
        "sbb %1, %0\n\t"
...
#ifdef _MSC_VER
      _subborrow_u8(0, GR8(Rd), K, &GR8(Rd));
#else
      __asm__ __volatile__("sub %1, %0\n\t" : "+r"(GR8(Rd)) : "r"(K) : "cc");
#endif
...
#ifdef _MSC_VER
      _subborrow_u8(GETF_CARRY(), GR8(Rd), K, &GR8(Rd));
#else
      __asm__ __volatile__(
        "bt %3, %2\n\t"
        "sbb %1, %0\n\t"
    ...
    case OP_CALL:
      ...
      STACK               = STACK - 2;
      PC                  = ((flash_.at(PC).instruction & 0x01) << 16) |
           ((flash_.at(PC).instruction & 0x1F0) << 13) |
           flash_.at(PC + 1).instruction;
      CC = CC + 4;
      break;
    case OP_JMP:
      PC = ((flash_.at(PC).instruction & 0x01) << 16) |
    ...
    case OP_POP:
      Rd      = (instruction >> 4) & 0x1F;
      STACK   = STACK + 1;
      GR8(Rd) = sram_.at(STACK);
      PC      = PC + 1;
      CC      = CC + 2;
      break;
    case OP_PUSH:
      Rd              = (instruction >> 4) & 0x1F;
      sram_.at(STACK) = GR8(Rd);
      STACK           = STACK - 1;
      PC              = PC + 1;
      CC              = CC + 2;
      break;
...
#ifdef _MSC_VER
      _subborrow_u8(0, GR8(Rd), GR8(Rr), &nu.u8);
#else
      __asm__ __volatile__("sub %1, %0\n\t" ::"r"(GR8(Rd)), "r"(GR8(Rr))
                           : "cc");
#endif
...
#ifdef _MSC_VER
      _subborrow_u8(GETF_CARRY(), GR8(Rd), GR8(Rr), &nu.u8);
#else
      __asm__ __volatile__(
        "bt %3, %2\n\t"
        "sbb %1, %0\n\t" ::"r"(GR8(Rd)),
...
#ifdef _MSC_VER
      _subborrow_u8(0, GR8(Rd), K, &nu.u8);
#else
      __asm__ __volatile__("sub %1, %0\n\t" ::"r"(GR8(Rd)), "r"(K) : "cc");
#endif
...
#ifdef _MSC_VER
      _addcarry_u16(0, GR16(Rd), (uint16_t)K, &GR16(Rd));
#else
      __asm__ __volatile__("add %1, %0\n\t"
                           : "+r"(GR16(Rd))
                           : "r"((uint16_t)K)
...
#ifdef _MSC_VER
      _subborrow_u16(0, GR16(Rd), (uint16_t)K, &GR16(Rd));
#else
      __asm__ __volatile__("sub %1, %0\n\t" : "+r"(GR16(Rd)) : "r"((uint16_t)K)
                           : "cc");
#endif
    ...
    case OP_BRCC:  // case OP_BRSH:
      if (SREG & (1 << C)) {
        ...
        PC     = (offset & (1 << 6)) ? PC - (((~offset) & 0x7F) + 1) + 1
                                     : PC + offset + 1;
        normalize_pc();
        CC = CC + 2;
      }
    ...
    case OP_STS:
      reg_n                                   = (instruction >> 4) & 0x1F;
      sram_.at(flash_.at(PC + 1).instruction) = GR8(reg_n);
      PC                                      = PC + 2;
      CC                                      = CC + 2;
      break;
    case OP_LDS:
      reg_n      = (instruction >> 4) & 0x1F;
      GR8(reg_n) = sram_.at(flash_.at(PC + 1).instruction);
      PC         = PC + 2;
      CC         = CC + 2;
      break;
      // ST and STD
    ...
    case OP_STD_Y:
      Rr = (instruction >> 4) & 0x1F;
      q  = ((instruction & 8192) >> 8) | ((instruction & 3072) >> 7) |
          (instruction & 0x07);
      sram_.at(IndexRegY + q) = GR8(Rr);
      PC                      = PC + 1;
      CC                      = CC + 2;
      break;
    case OP_ST_Z:
      Rr                  = (instruction >> 4) & 0x1F;
      sram_.at(IndexRegZ) = GR8(Rr);
      PC                  = PC + 1;
      CC                  = CC + 2;
      break;
    ...
    case OP_LDD_Y:
      ...
      q  = ((instruction & 8192) >> 8) | ((instruction & 3072) >> 7) |
          (instruction & 0x07);
      GR8(Rr) = sram_.at(IndexRegY + q);
      PC      = PC + 1;
      CC      = CC + 3;
      break;
    case OP_LD_Z:
      Rr      = (instruction >> 4) & 0x1F;
      GR8(Rr) = sram_.at(IndexRegZ);
      PC      = PC + 1;
      CC      = CC + 1;
      break;
    ...
    case OP_LPM:
      tmp16 = IndexRegZ;
      if (tmp16 >= flash_.size() * 2) throw invalid_lpm_source();
      GR8(0) = flash_.get_byte(tmp16);
      PC     = PC + 1;
      CC     = CC + 3;
      break;
    case OP_LPM_R:
    ...
    case OP_MUL:
      ...
      Rd = (instruction >> 4) & 0x1F;
      Rr = (instruction & 0x0F) | ((instruction & 0x200) >> 5);
      *static_cast<uint16_t*>(R0R1_PTR) = (uint8_t)GR8(Rd) * (uint8_t)GR8(Rr);
    ...
    case OP_MULS:
      ...
      *static_cast<int16_t*>(R0R1_PTR) = (int8_t)GR8(Rd) * (int8_t)GR8(Rr);
    ...
    case OP_MULSU:
      ...
      *static_cast<int16_t*>(R0R1_PTR) = (int8_t)GR8(Rd) * (uint8_t)GR8(Rr);
    ...
    case OP_FAKE_UDR_OUT:
      *ucsra_ptr_ |= (1 << 6);  // set TXC
      if (mstate_) {
        mstate_->uart_put_byte(GR8(instruction & 0x1F), dev_addr_);
    ...
    case OP_FAKE_DEBUG: {
      ...
      if (code == 0) {
        ...
        auto frame =
          reinterpret_cast<net::Frame*>(&sram_[(addr == 2 ? 0xdd : 0xe9)]);
        std::cout << std::hex;
...
SIM_IMPORT_EXPORT void AVRCore::PrintLastInstructions()
{
  ...
  for (ri i = last_pc_.rbegin(); i != last_pc_.rend(); i++, k--) {
    std::cout << "\n (pc*2) " << std::setfill(' ') << std::setw(3) << std::dec
              << k << ": " << std::hex << std::setfill('0') << std::setw(4)
              << (*i) * 2;
  }
  std::cout << std::endl;
  std::cout.flags(f);
