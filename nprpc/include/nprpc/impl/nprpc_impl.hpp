...
template<typename StreamType> class WebSocketConnectionImpl;

// Type aliases for the two kinds of WebSocket connections
using ws_ssl_stream = boost::beast::websocket::stream<
  boost::asio::ssl::stream<boost::beast::tcp_stream>>;
using ws_plain_stream = boost::beast::websocket::stream<
  boost::beast::tcp_stream>;

using WebSocketSSLConnection = WebSocketConnectionImpl<ws_ssl_stream>;
using WebSocketPlainConnection = WebSocketConnectionImpl<ws_plain_stream>;

template <typename StreamType>
class WebSocketConnectionImpl 
  : public Session 
  , public CommonConnection<WebSocketConnectionImpl<StreamType>>
  , public std::enable_shared_from_this<WebSocketConnectionImpl<StreamType>>
{
  StreamType ws_;
  boost::beast::flat_buffer read_buffer_;
  boost::asio::ip::tcp::endpoint endpoint_;
  std::unique_ptr<boost::asio::ssl::context> ssl_ctx_; // Only used for SSL connections
  ...
  void async_write_impl(
    Stream& stream,
    const flat_buffer& buffer,
    std::function<void(const boost::system::error_code&, size_t)> handler)
  {
    stream.async_write(
      boost::asio::buffer(buffer.data()), std::move(handler));
  }
  ...
  void async_read_impl(
    ...
    std::function<void(const boost::system::error_code&, size_t)> handler)
  {
    stream.async_read(read_buffer_, std::move(handler));
  }
...
public:
  ...
  WebSocketConnectionImpl(const EndPoint& endpoint, boost::asio::ip::tcp::socket&& socket);
 ...
 public:
    ...
    SessionContext* ctx)
  {
    ...
    if (activation_flags & ObjectActivationFlags::ALLOW_TCP) {
      oid.urls +=
        (std::string(tcp_prefix) + default_url + ":" + std::to_string(g_cfg.port)) + ';';
    }

    if (activation_flags & ObjectActivationFlags::ALLOW_WEBSOCKET) {
      oid.urls +=
        (std::string(ws_prefix) + default_url + ":" + std::to_string(g_cfg.websocket_port)) +
        ';';
    }
    ...
    if (activation_flags & ObjectActivationFlags::ALLOW_SSL_WEBSOCKET) {
      ...
      oid.urls += (std::string(wss_prefix) + g_cfg.hostname + ":" +
                   std::to_string(g_cfg.websocket_port));
    }
...
{
  assert(id == MessageId::Success || id == MessageId::Error_ObjectNotExist ||
         id == MessageId::Error_CommFailure ||
         id == MessageId::Error_UnknownFunctionIdx ||
         id == MessageId::Error_UnknownMessageId ||
         id == MessageId::Error_BadAccess || id == MessageId::Error_BadInput);
