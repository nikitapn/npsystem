...
enum Enum : uint32_t {
  ...
  NONE                     = 0,
  // Means that evil hackers will be able to get access to this object from any other session
  // besides from the one it was created in
...
class RpcBuilder {
 ...
 public:
  ...
  RpcBuilder& set_port(uint16_t port) noexcept
  {
    cfg_.port = port;
    return *this;
  }

  RpcBuilder& set_websocket_port(uint16_t port) noexcept
  {
    cfg_.websocket_port = port;
    return *this;
  }
  ...
  RpcBuilder& set_use_ssl(bool use_ssl) noexcept
  {
    cfg_.use_ssl = use_ssl;
    return *this;
  }

  RpcBuilder& set_ssl_public_key(std::string_view key) noexcept
  {
    cfg_.ssl_public_key = key;
    return *this;
  }

  RpcBuilder& set_ssl_secret_key(std::string_view key) noexcept
  {
    cfg_.ssl_secret_key = key;
    return *this;
  }

  RpcBuilder& set_ssl_dh_params(std::string_view params) noexcept
  {
    cfg_.ssl_dh_params = params;
    return *this;
  }
