...
using tcp_stream_strand = net::basic_stream_socket<net::ip::tcp, net::strand<net::io_context::executor_type>>;
using beast_tcp_stream_strand = beast::basic_stream<net::ip::tcp, net::strand<net::io_context::executor_type>>;
...
class plain_websocket_session
  ...
  websocket::stream<beast_tcp_stream_strand> ws_;

public:
  // Create the session
  explicit plain_websocket_session(beast_tcp_stream_strand &&stream)
    : websocket_session<plain_websocket_session>(stream.get_executor()), ws_(std::move(stream)) {
  ...
  websocket::stream<beast_tcp_stream_strand> &ws() { return ws_; }
};
...
class ssl_websocket_session
  ...
  websocket::stream<beast::ssl_stream<beast_tcp_stream_strand>> ws_;

public:
  // Create the ssl_websocket_session
  explicit ssl_websocket_session(beast::ssl_stream<beast_tcp_stream_strand> &&stream)
    : websocket_session<ssl_websocket_session>(stream.get_executor()), ws_(std::move(stream)) {
  ...
  // Called by the base class
  websocket::stream<beast::ssl_stream<beast_tcp_stream_strand>> &ws() { return ws_; }
};
...
void init_web_socket(boost::asio::io_context &ioc) {
  ...
  std::make_shared<listener>(
    ...
    std::make_shared<std::string const>(nprpc::impl::g_cfg.http_root_dir)
  )->run();
}
