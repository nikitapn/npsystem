...
try:
    ...
    # Find the line with "footstep: in Footstep"
    # Line 9 (0-indexed line 8): "  void SendFootstep(footstep: in Footstep);"
    # Click on "Footstep" type at the end
    test_line = 8  # 0-indexed
    # Find position of the type "Footstep" (after "in ")
    line_text = lines[test_line]
    type_pos = line_text.rfind('Footstep')  # Last occurrence
    ...
    if resp and 'result' in resp and resp['result']:
        ...
        if isinstance(result, dict) and 'range' in result:
            ...
            if 'Footstep : flat' in lines[target_line]:
                ...
                print(f"   This indicates the parameter range is too large")
            else:
