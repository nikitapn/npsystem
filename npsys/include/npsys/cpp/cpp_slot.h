...
std::vector<std::string_view> CFBDSlot::split_path(std::string_view str) {
...
fbd_slot_n CFBDSlot::get_by_path(std::string_view path) {
	auto tok = split_path(path);


	if (tok.size() < 3) throw std::runtime_error("Invalid link");

	std::string_view& cat_name = tok[0];
	std::string_view& alg_name = tok[1];
	std::string_view& slot_full_name = tok[2];
	...
	if (!cat) {
		throw std::runtime_error("There is no such category \"" + std::string(cat_name) + '\"');
	}
	...
	for (auto& block : blocks) {
		if (block->get_name() == block_name) {
			...
			throw std::runtime_error("There is no such slot \"" + std::string(slot_full_name) + '\"');
		}
	}
	throw std::runtime_error("There is no such slot \"" + std::string(slot_full_name) + '\"');
}
