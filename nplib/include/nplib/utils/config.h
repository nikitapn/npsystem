...
class config_base
{
  ...
  template <typename U>
  friend std::ostream& ::operator<<(std::ostream&, const config_base<U>&);

 public:
  const auto& config() const noexcept { return config_; }
  const auto& data_dir() const noexcept { return data_dir_; }
  const auto& assets_dir() const noexcept { return assets_dir_; }
    ...
    std::string_view module_name) noexcept
  {
    ...
    PWSTR   ppszPath;
    HRESULT hr =
      SHGetKnownFolderPath(FOLDERID_RoamingAppData, NULL, NULL, &ppszPath);
    assert(hr == S_OK);
    ...
    auto getExecutableRootPath = []() noexcept {
      ...
      return std::filesystem::path(
        std::string(buffer.get(), c - buffer.get() + 1));
    };
    assets_dir_ = getExecutableRootPath() / "data";
#elif __linux__
    data_dir_   = "/var/local/npsystem";
    assets_dir_ = "/usr/local/share/npsystem";
    config_     = "/etc/npsystem";
    config_ /= module_name;
    config_ += ".cfg";
#else
#error "platform is not supported"
    ...
    if (!stream) {
      mp11::mp_for_each<mp11::mp_iota_c<fuz::result_of::size<T>::value>>(
        [&](auto I) {
          fuz::at_c<static_cast<int>(I)>(*this_) =
            std::get<I>(this_->traits).default_value;
        });
      std::cerr << clr::color::red
                << "config file is missing: " << clr::color::reset << config_
                << '\n';
    } else {
      boost::property_tree::ptree pt;
      boost::property_tree::ini_parser::read_ini(stream, pt);
      mp11::mp_for_each<
        mp11::mp_iota_c<fuz::result_of::size<T>::value>>([&](auto I) {
        auto name =
          std::string(
            detail::sec<typename std::tuple_element_t<I, decltype(T::traits)>::
                          section>::name) +
          '.' +
          std::string(
            fuz::extension::struct_member_name<T, static_cast<int>(I)>::call());
        auto& val = fuz::at_c<static_cast<int>(I)>(*this_);
        try {
          val = pt.get<std::remove_reference_t<decltype(val)>>(name);
        } catch (...) {
          std::cerr << "error while reading: " << name
                    << " default value will be using.\n";
...
std::ostream& operator<<(
  std::ostream& os, const nplib::config::config_base<T>& cfg)
{
  namespace fuz  = boost::fusion;
  namespace mp11 = boost::mp11;
  os << "Config:\n";
  os << "\tconfig" << " = " << cfg.config_ << '\n';
  os << "\tdata_dir" << " = " << cfg.data_dir_ << '\n';
  os << "\tassets_dir" << " = " << cfg.assets_dir_ << '\n';
  mp11::mp_for_each<mp11::mp_iota_c<fuz::result_of::size<T>::value>>(
    [&](auto I) {
      auto name = std::string(
        fuz::extension::struct_member_name<T, static_cast<int>(I)>::call());
      auto const& val =
        fuz::at_c<static_cast<int>(I)>(static_cast<const T&>(cfg));
      os << '\t' << name << " = " << val << '\n';
    });
  return os;
}
...
#define NPLIB_MAKE_DEFAULT_VALUE_TYPE(R, ATTRIBUTE_TUPLE_SIZE, I, ATTRIBUTE) \
  BOOST_PP_COMMA_IF(I)                                                       \
  nplib::config::ValueTrait<nplib::config::sections::BOOST_PP_TUPLE_ELEM(    \
                              2, 0, BOOST_PP_TUPLE_ELEM(3, 2, ATTRIBUTE)),   \
                            BOOST_PP_TUPLE_ELEM(3, 0, ATTRIBUTE)>

#define NPLIB_MAKE_DEFAULT_VALUE(R, ATTRIBUTE_TUPLE_SIZE, I, ATTRIBUTE) \
  BOOST_PP_COMMA_IF(I)                                                  \
  BOOST_PP_TUPLE_ELEM(2, 1, BOOST_PP_TUPLE_ELEM(3, 2, ATTRIBUTE))
...
#define BOOST_FUSION_DEFINE_STRUCT_PLUS_ENUM_IMPL(                           \
  ...
  BOOST_FUSION_ADAPT_STRUCT_NAMESPACE_DEFINITION_BEGIN(NAMESPACE_SEQ)        \
                                                                             \
  struct NAME : public nplib::config::config_base<NAME> {                    \
    typedef NAME self_type;                                                  \
    static constexpr std::tuple<BOOST_PP_SEQ_FOR_EACH_I_R(                   \
      1,                                                                     \
      NPLIB_MAKE_DEFAULT_VALUE_TYPE,                                         \
      ATTRIBUTE_TUPLE_SIZE,                                                  \
      BOOST_PP_SEQ_TAIL(ATTRIBUTES_SEQ))>                                    \
      traits {BOOST_PP_SEQ_FOR_EACH_I_R(1,                                   \
...
#define BOOST_FUSION_ADAPT_STRUCT_ATTRIBUTES_FILLER_MY(VA_ARGS_SEQ)            \
  BOOST_PP_SEQ_PUSH_FRONT(                                                     \
    BOOST_PP_SEQ_FOR_EACH(                                                     \
      BOOST_FUSION_ADAPT_STRUCT_ATTRIBUTES_FILLER_OP_MY, unused, VA_ARGS_SEQ), \
    (0, 0))
