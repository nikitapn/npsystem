...
CFBDControlUnit::CFBDControlUnit(std::string&& name, npsys::algorithm_category_n& cat) {
	this->set_name(name);
	this->editor_.create();
...
NPSystemIcon CFBDControlUnit::GetIcon() const noexcept {
	return NPSystemIcon::Algorithm;
}

CBlockComposition* CFBDControlUnit::GetBlocks() {
...
void CFBDControlUnit::HardwareSpecific_Clear() {
...
void CFBDControlUnit::Save(bool after_uploading) noexcept {
...
void CFBDControlUnit::ClearVariables() noexcept {
...
std::optional<npsys::fbd_slot_n>
CFBDControlUnit::FindSlot(int slotId) {
	fbd_blocks.fetch_all_nodes();
	for (auto& b : fbd_blocks) {
		b->slots.fetch_all_nodes();
		for (auto& s : b->slots) {
...
void CFBDControlUnit::DetermineTypes(npsys::fbd_control_unit_n& self) {
	if (m_running) return;
...
void CFBDControlUnit::Separate(std::vector<CBlock*>* pV, CBlock* block, char* color) {
	pV->push_back(block);
...
int CFBDControlUnit::CalcOrder(CBlock* block, int order, char* color) {
...
int	CFBDControlUnit::TryExecuteNext(CBlock* block, int order, char* color) {
...
void CFBDControlUnit::Translate(CCodeGenerator* pGenerator) {
	pGenerator->Reset();
...
bool CFBDControlUnit::IsUndoRedo() {
	return editor_.loaded()
		&& (editor_->IsRedoCommands() || editor_->IsUndoCommands());
}

void CControlUnit::CloseView() {
	StopOnline();
	// __super::CloseView();
}

void CFBDControlUnit::DrawOnlineValues(CGraphics* pGraphics) {
	if (!online_) return;
	for (auto lo : m_loadedSlots) lo->DrawOnlineValue(pGraphics);
}

void CFBDControlUnit::StartOnline() {
	if (GetStatus() == Status::status_not_assigned) {
		MessageBoxA(g_hMainWnd, "Algorithm is not assigned", "", MB_ICONINFORMATION);
		return;
	} else if (GetStatus() == Status::status_assigned) {
		MessageBoxA(g_hMainWnd, "Algorithm is not loaded", "", MB_ICONINFORMATION);
		return;
	} else if (GetStatus() == Status::status_not_actual) {
...
void CFBDControlUnit::UploadToDevice(bool mb_confirm) noexcept {
	if (GetStatus() < npsys::CControlUnit::Status::status_assigned) {
	...
	auto self = self_node.fetch();
	auto self2 = self.cast<fbd_control_unit_n>();
	using Task = CDlg_LoadingManager::CTaskLoadOneAlgorithm;
	CDlg_LoadingManager manager(
		std::make_unique<Task>(device, self2));
	manager.DoModal(g_hMainWnd);
}

void CFBDControlUnit::UnloadFromDevice() noexcept {
	if (GetStatus() < npsys::CControlUnit::Status::status_loaded) {
...
void CFBDControlUnit::DeletePermanent() noexcept {
	editor_.remove();
...
void CFBDControlUnit::StopOnline() {
	if (!online_) return;
...
#ifdef DEBUG
void CFBDControlUnit::ShowBlocks() {
	std::cout << get_name() << ":\n Slots:\n";
	fbd_blocks.fetch_all_nodes();
	for (auto& block : fbd_blocks) {
...
void CControlUnit::OnlineRestart() {
	if (!IsOnline()) return;
...
size_t CFBDControlUnit::AdviseImpl() {
	auto device = assigned_dev.fetch();
	if (!device.loaded()) {
		assert(false);
		return 0;
...
void CFBDControlUnit::UpdateData(std::vector<nps::server_value>* upd) {
	if (upd) {
...
void CFBDControlUnit::OnConnectionLost() noexcept {
