...
void ItemManagerImpl::keep_alive_timer(
  std::shared_ptr<std::atomic_bool> /* del */)
{
...
// nprpc thread
void ItemManagerImpl::Advise(
  nprpc::flat::Span_ref<nps::flat::DataDef, nps::flat::DataDef_Direct> a,
  /*out*/ nprpc::flat::Vector_Direct1<uint64_t>                        h)
{
  h.length(a.size());
  auto span = h();
  t_advise({(nps::flat::DataDef*)a.data(), (nps::flat::DataDef*)a.data_end()},
           span);
}

// nprpc thread
void ItemManagerImpl::UnAdvise(
  nprpc::flat::Span<uint64_t> /* a */)
{
}

// nprpc thread
void ItemManagerImpl::Activate(
  nprpc::Object* client)
{
  assert(client);
...
void ItemManagerImpl::destroy() noexcept
{
  ...
  nplib::async<false>(
    strand_, [this]() { destroy_from_strand(); });  // last job in this strand
}
...
void ItemManagerImpl::destroy_from_strand() noexcept
{
  if (processing_on_data_changed_) {
    std::cerr << "ItemManagerImpl::destroy_from_strand().should not happen - "
                 "processing_on_data_changed_ = true\n";
    nplib::async<false>(strand_, [this]() { destroy_from_strand(); });
    return;
  }
  if (g_cfg.log_level > 1) {
    std::cout << "ItemManagerImpl::destroy_from_strand() "
              << "task_count: " << task_count() << std::endl;
  }
  pd_client_.reset();
  delete this;
}
...
// strand context
void ItemManagerImpl::OnDataChanged(
  const std::vector<nps::server_value>& ar)
{
...
void ItemManagerImpl::fire_keep_alive()
{
  ...
  try {
    ...
    timer_.expires_from_now(
      boost::posix_time::seconds(g_cfg.client_keep_alive_time), ec);
  } catch (nprpc::Exception& ex) {
